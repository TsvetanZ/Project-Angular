...
import { BehaviorSubject, catchError, combineLatest, Observable, of, switchMap, take, throwError, withLatestFrom, zip } from "rxjs";
...
export class AppInterceptor implements HttpInterceptor {
    ...
    intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        if(req.url.startsWith('/api')) {
            req = req.clone({ url: req.url.replace('/api', apiUrl), withCredentials: true}); // withCredentials: true ако има кукита праща ги до бакента да се използват
        }
